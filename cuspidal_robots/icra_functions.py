...
def get_Background(title=None, page=None):
    bottom_title = Tex(r"Design optimization of a parallel manipulator for otological surgery").scale(0.64).move_to(
        [0, -3.7, 0])
    bottom_line = Line(start=[-7, -3.5, 0], end=[7, -3.5, 0], stroke_width=0.5)
    top_line = Line(start=[-7, 3.2, 0], end=[7, 3.2, 0], stroke_width=0.5)
    # icra_L2 = ImageMobject("icra_logo2.png").scale(0.05).move_to([-6, 3.5, 0])
    icra_L2 = ImageMobject("../2UPS/cfm_logo.png").scale(0.32).move_to([-6, 3.5, 0])
    top_title = None
    page_num = None
    ...
    if title is None and page is None:
        return bottom_title, bottom_line, top_line, icra_L2
    elif title is None:
        return bottom_title, bottom_line, top_line, icra_L2, page_num
    elif page is None:
        return bottom_title, bottom_line, top_line, icra_L2, top_title
    else:
        return bottom_title, bottom_line, top_line, icra_L2, page_num, top_title
...
def get_implicitplot(paras=None, rdw=None):
    ...
    if rdw == "circ":
        circ1 = Circle(radius=1, stroke_width=1, color=GREEN, fill_color=GREEN, fill_opacity=0.6).move_to(c.get_center())
